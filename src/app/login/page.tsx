...
export default function LoginPage() {
  ...
  const personas = [
    ...
    },
    {
      ...
      icon: <HeartPulse className="w-5 h-5 text-cyan-500" />,
  ...
  return (
    <div className="min-h-screen bg-[#070b14] text-slate-100 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center z-10">
        <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-tr from-teal-600 to-cyan-500 text-white shadow-xl shadow-teal-500/20 mb-4 ring-1 ring-white/20">
          <Stethoscope className="w-8 h-8" />
        </div>
        <h1 className="text-3xl font-extrabold tracking-tight text-white flex items-center justify-center gap-2">
          MEDORA
          <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-teal-500/20 text-teal-400 border border-teal-500/30 uppercase tracking-widest">
            Care
          </span>
        </h1>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-4xl z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 bg-slate-900/90 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-2xl">
          ...
          >
            <div>
              <Input
                ...
                type="email"
                placeholder="clinician@medora.health"
                value={email}
          ...
          <div className="mt-6 pt-5 border-t border-slate-800/80 flex items-start gap-2 text-[11px] text-slate-500 leading-relaxed">
            ...
            <span>
              For authorized clinical monitoring only. Does not replace professional clinical judgment.
            </span>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 bg-slate-900/60 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-6 sm:p-8 shadow-xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            {personas.map((p) => (
              <button
                ...
                disabled={isLoading}
                className="text-left p-4 rounded-xl bg-slate-950/60 hover:bg-slate-800/80 border border-slate-800 hover:border-teal-500/50 transition-all group relative overflow-hidden"
              >
          ...
          <div className="mt-4 p-3 rounded-xl bg-teal-950/30 border border-teal-800/40 text-xs text-teal-300/80 flex items-center justify-between">
            <span>✨ All demo accounts pre-seeded with rich clinical cases.</span>
            <span className="text-[11px] text-teal-400 font-mono">PWD: Medora2026!</span>
          </div>
        </div>
      </div>
    </div>
